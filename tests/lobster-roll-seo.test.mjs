...
assert.match(html, /<img[^>]+width="1081"[^>]+height="1280"[^>]+fetchpriority="high"/);
assert.doesNotMatch(html, /hot butter|\+\$2/i);
assert.match(html, /60\+ bourbons/);
assert.match(html, /Shepherd's pie, chicken pot pie and Dave-oux gumbo/);
...
assert.equal(webPage.mainEntity.length, 4, 'visible FAQ must have four matching structured questions');
